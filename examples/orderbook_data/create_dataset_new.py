...
"""
    ...
    - TODO: Its structure is not well designed, your contribution is welcome to make importing dataset easier
"""
...
def get_library_name(doc_type):
    if str.upper(doc_type) == str.upper('Tick'):
        return 'TICKS'
    else:
        return str.upper(doc_type)
...
def add_one_stock_daily_data(filepath, type, exchange_place, arc, date):
    ...
    def format_time(day, hms):
        ...
        hms = str(hms)
        if hms[0] == '1':  # >=10,
            return "-".join([day[0:4], day[4:6], day[6:8]]) + " " + ":".join(
                [hms[:2], hms[2:4], hms[4:6] + "." + hms[6:]])
        else:
            return "-".join([day[0:4], day[4:6], day[6:8]]) + " " + ":".join(
                [hms[:1], hms[1:3], hms[3:5] + "." + hms[5:]])
...
def add_data(tick_date, doc_type, stock_name_dict):
    ...
    try:
        ...
        os.system(f"tar -xvzf {DATA_PATH}/{tick_date + '_{}.tar.gz'.format(doc_type)} -C {DATA_PATH}/ {tick_date + '_' + doc_type}/SH")
        os.system(f"tar -xvzf {DATA_PATH}/{tick_date + '_{}.tar.gz'.format(doc_type)} -C {DATA_PATH}/ {tick_date + '_' + doc_type}/SZ")
        ...
        sz_files = (set([i.split(".csv")[0] for i in os.listdir(temp_data_path_sz) if i[:2] == '30' or i[0] == '0'])
                    & set(stock_name_dict['SZ'])) if is_files_exist['sz'] else set()
        sz_file_nums = len(sz_files) if is_files_exist['sz'] else 0
        sh_files = (set([i.split(".csv")[0] for i in os.listdir(temp_data_path_sh) if i[0] == '6'])
                    & set(stock_name_dict['SH'])) if is_files_exist['sh'] else set()
        ...
        if sh_file_nums > 0:
            ...
            Parallel(n_jobs=N_JOBS)(
                delayed(add_one_stock_daily_data_wrapper)(os.path.join(temp_data_path_sh, name +
                                                                       ".csv"), doc_type, "SH", index, tick_date)
                for index, name in enumerate(list(sh_files)))
        if sz_file_nums > 0:
            # write线程不安全, update应该是安全的
            Parallel(n_jobs=N_JOBS)(
                delayed(add_one_stock_daily_data_wrapper)(os.path.join(temp_data_path_sz, name +
                                                                       ".csv"), doc_type, "SZ", index, tick_date)
    ...
    def import_data(self):
        ...
        for stock_name in stock_name_list:
            if stock_name not in stock_name_exist:
                initialize_count += 1
                lib.write(stock_name, DataFrame({"date": ["1900-01-01"]}))
        ...
        Parallel(n_jobs=min(2, len(date_list)))(delayed(add_data)(date, doc_type, stock_name_dict) for date in date_list)
