...
class DDGDA:
    ...
    **NOTE**
    ...
    def get_feature_importance(self):
        ...
        # So we must use following extra steps to get the right feature importance
    ...
    def dump_data_for_proxy_model(self):
        """
        Dump data for training metamodel.
        The metamodel will be trained upon the proxy forecasting model.
    ...
    def dump_meta_ipt(self):
        """
        Dump data for training metamodel.
        This function will dump the input data for metaodel
        """
    ...
    def train_meta_model(self):
        """
        training a metamodel based on a simplified linear proxy model;
        """

        # 1) leverage the simplified proxy forecasting model to train metamodel.
        # - Only the dataset part is important, in current version of metamodel will integrate the
        rb = RollingBenchmark(model_type=self.sim_task_model)
        sim_task = rb.basic_task()
        ...
        # NOTE:
        # the input of metamodel (internal data) are shared between proxy model and final forecasting model
    ...
    def meta_inference(self):
        """
        Leverage metamodel for inference:
        - Given
            - baseline tasks
            - input for metamodel(internal data)
            - metamodel (its learnt knowledge on proxy forecasting model is expected to transfer to normal forecasting model)
        """
        # 1) get metamodel
        ...
        # 2)
        # we are transfer the knowledge of metamodel to final forecasting-tasks.
    ...
    def train_and_eval_tasks(self):
        """
        Training the tasks generated by metamodel
        Then evaluate it
        """
