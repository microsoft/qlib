...
def run(times=1, models=None, dataset="Alpha360", exclude=False):
    ...
    for fn in folders:
        ...
        if fn == "TFT":
            execute(
                f"cd {env_path} && {python_path} -m pip install --upgrade --force-reinstall --ignore-installed PyYAML -e git+https://github.com/bxdd/qlib#egg=pyqlib"
            )  # TODO: FIX ME!
        else:
            execute(
                f"cd {env_path} && {python_path} -m pip install --upgrade --force-reinstall -e git+https://github.com/bxdd/qlib#egg=pyqlib"
            )  # TODO: FIX ME!
        sys.stderr.write("\n")
