...
class SignalRecord(RecordTemp):
    ...
    def generate(self, **kwargs):
        """
        Generate the prediction and label, and save them.
...
class SigAnaRecord(SignalRecord):
    ...
    def generate(self, pred=None, label=None, **kwargs):
        """
        Generate the IC and Rank IC, and save them. If user wants to use their own prediction or label, please make sure they feed both the arguments, otherwise,
        this method will try to use the prediction and label generated by `SignalRecord`.
        ...
        label : pd.Series
            the label corresponding to the prediciton.
        kwargs

        Return
        ------
        """
        # generate prediction and label with SignalRecord
...
class PortAnaRecord(SignalRecord):
    ...
    def generate(self, pred=None, **kwargs):
        """
        Generate the backtest results and save them.
        ...
        report_dict = normal_backtest(pred, strategy=self.strategy, **self.backtest_config)
