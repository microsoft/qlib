...
class QLibDataHandlerV1(ConfigQLibDataHandler):
    ...
    def setup_label(self):
        ...
        df_labels["LABEL3"] = df_labels["LABEL2"].groupby(level="datetime").apply(lambda x: (x - x.mean()) / x.std())

        ## drop labels are not used
        df_labels = df_labels.drop(["LABEL0", "LABEL1", "LABEL2"], axis=1)
