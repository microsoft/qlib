...
class GATs_ADD_IND(Model):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.GAT_model = GATModel_ADD_IND(
            ...
            smooth_perplexity=self.smooth_perplexity,
        )
    ...
    def train_epoch(self, data_loader):
        ...
        for data in data_loader:
            ...
            # there is no nan in ind
            assert torch.isnan(ind).sum() == 0

            # adjacent_matrix = torch.ones((feature.shape[0],feature.shape[0])).float().to(self.device)
            adjacent_matrix = (ind.reshape(-1, 1) == ind.reshape(1, -1)).float().to(self.device)

            pred = self.GAT_model(feature.float(), adjacent_matrix)
    ...
    def predict(self, dataset):
        ...
        for data in test_loader:
            ...
            with torch.no_grad():
                pred = self.GAT_model(feature.float(), adjacent_matrix).detach().cpu().numpy()
