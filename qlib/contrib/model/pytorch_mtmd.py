...
class MTMD(nn.Module):
    ...
    def read(Q,K):
        softmax_score_query, softmax_score_memory = MTMD.get_score(K,Q)
    ...
    def train_epoch(self, x_train, y_train, stock_index):
        ...
        for idx, count in zip(daily_index, daily_count):
            ...
            pred = self.MTMD_model(feature, concept_matrix)
    ...
    def fit(
        ...
        def predict(self, dataset: DatasetH, segment: Union[Text, slice] = "test"):
            ...
            for idx, count in zip(daily_index, daily_count):
                ...
                with torch.no_grad():
                    pred = self.MTMD_model(x_batch, concept_matrix).detach().cpu().numpy()
...
class MTMDModel(nn.Module):
    ...
    def forward(self, x, concept_matrix, market_value,m_items,train):
        ...
        concept_to_stock = MTMD.cal_cos_similarity(x_hidden, hidden)
        ...
        p_shared_info, _,softmax_score_memory0 = MTMD.read(p_shared_info,m_item0)
        if train:
            softmax_score_memory0 = MTMD.upload(p_shared_info,m_item0)
        else:
        ...
        h_stock_to_concept = MTMD.cal_cos_similarity(hidden, hidden)
        ...
        h_concept_to_stock = MTMD.cal_cos_similarity(h_shared_info, hidden)
        ...
        h_shared_info, _,softmax_score_memory1 = MTMD.read(h_shared_info,m_item1)
        
        if train:
            softmax_score_memory1 = MTMD.upload(h_shared_info,m_item1)
        else:
