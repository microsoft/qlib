...
from .interpreter import FullHistoryATObs
...
class Recurrent(nn.Module):
    ...
    def _source_features(self, obs: FullHistoryATObs, device: torch.device) -> Tuple[List[torch.Tensor], torch.Tensor]:
        ...
        position = obs["position_history"].sign()
        steps = (
        ...
        dir_out = self.dire_fc(torch.stack((obs["position"], -obs["position"]), -1).float())
    ...
    def forward(self, batch: Batch) -> torch.Tensor:
        ...
        inp = cast(FullHistoryATObs, batch)
