...
from qlib.rl.algorithm_trading import SingleAssetAlgorithmTradingSimple
...
def _read_tasks(task_dir: Path) -> pd.DataFrame:
    if os.path.isfile(task_dir):
        return pd.read_pickle(task_dir)
    else:
        tasks = []
        for file in task_dir.iterdir():
            task_data = pd.read_pickle(file)
...
class LazyLoadDataset(Dataset):
    def __init__(
        ...
        self._task_df = _read_tasks(task_file_path).reset_index()

        self._data_dir = data_dir
        self._ticks_index: Optional[pd.DatetimeIndex] = None

    def __len__(self) -> int:
        return len(self._task_df)

    def __getitem__(self, index: int) -> Task:
        row = self._task_df.iloc[index]
...
def train_and_test(
    ...
    task_root_path = Path(data_config["source"]["task_dir"])

    data_granularity = simulator_config.get("data_granularity", 1)

    def _simulator_factory_simple(task: Task) -> SingleAssetAlgorithmTradingSimple:
        return SingleAssetAlgorithmTradingSimple(
            task=task,
            data_dir=Path(data_config["source"]["data_dir"]),
            ticks_per_step=simulator_config["time_per_step"],
            data_granularity=data_granularity,
            fee_rate=simulator_config["fee_rate"],
    ...
    if run_training:
        train_dataset, valid_dataset = [
            LazyLoadDataset(
                task_file_path=task_root_path / tag,
                data_dir=Path(data_config["source"]["data_dir"]),
        ...
        train(
            ...
            policy=policy,
            reward=reward,
            initial_states=cast(List[Task], train_dataset),
            trainer_kwargs={
    ...
    if run_backtest:
        test_dataset = LazyLoadDataset(
            task_file_path=task_root_path / "test",
            data_dir=Path(data_config["source"]["data_dir"]),
            default_start_time_index=data_config["source"]["default_start_time_index"] // data_granularity,
            default_end_time_index=data_config["source"]["default_end_time_index"] // data_granularity,
        )
