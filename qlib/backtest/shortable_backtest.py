    ...
    def _execute_orders(self, trade_decision: TradeDecisionWO, date: pd.Timestamp):
        ...
        if self._is_trading_day(date) and isinstance(self.account.current_position, ShortablePosition):
            ...
            borrow_cost = self.borrow_fee_model.calculate_daily_cost(
                stock_positions,  # Now with current daily prices
                date
            )
...
class LongShortStrategy:
    ...
    def generate_trade_decision(self,
                               signal: pd.Series,
                               current_position: ShortablePosition,
                               date: pd.Timestamp) -> TradeDecisionWO:
        """
        ...
        short_prices = self._get_current_prices(short_stocks, date, self.exchange, OrderDir.SELL) if short_stocks else {}
    ...
    def _check_risk_limits(self, orders: List[Order], position: ShortablePosition) -> bool:
        ...
        return (leverage <= self.risk_limit["max_leverage"] and 
                abs(net_exposure) <= self.risk_limit["max_net_exposure"])
    ...
    def _simulate_position_change(self, orders: List[Order], position: ShortablePosition) -> ShortablePosition:
        ...
        stock_positions = {sid: {"amount": position.get_stock_amount(sid),
        ...
        for od in orders:
            ...
            if not _valid(price) and getattr(self, "trade_exchange", None) is not None and hasattr(od, "start_time"):
                try:
                    px = self.trade_exchange.get_deal_price(od.stock_id, od.start_time, 
                                                           od.end_time or od.start_time, od.direction)
                    if _valid(px): 
                        price = float(px)
                except Exception:
                    pass
