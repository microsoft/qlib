    ...
    def save_instruments(self, instruments_data: Union[list, pd.DataFrame]):
        ...
        if isinstance(instruments_data, pd.DataFrame):
            ...
            if self._inst_prefix:
                ...
                instruments_data[self.SAVE_INST_FIELD] = instruments_data[
                    self.symbol_field_name
                ].apply(lambda x: f"{self._inst_prefix}{x}")
    ...
    def _data_to_bin(
        ...
        for field in self.get_dump_fields(_df.columns):
            bin_path = features_dir.joinpath(
                f"{field}.{self.freq}{self.DUMP_FILE_SUFFIX}"
            )
...
class DumpDataFix(DumpDataAll):
    def _dump_instruments(self):
        ...
        new_stock_files = sorted(
            filter(lambda x: x.name not in self._old_instruments, self.csv_files)
        )
...
class DumpDataUpdate(DumpDataBase):
    def __init__(
        ...
        self._exclude_fields = tuple(
            set(self._exclude_fields) | {self.symbol_field_name}
        )
    ...
    def _load_all_source_data(self):
        ...
        def _read_csv(file_path: Path):
            if self._include_fields:
                _df = pd.read_csv(
                    file_path,
                    usecols=self._include_fields,
                    parse_dates=[self.date_field_name],
                )
            else:
        ...
        with ProcessPoolExecutor(max_workers=self.works) as executor:
            ...
            for _code, _df in self._all_data.groupby(self.symbol_field_name):
                ...
                if _code in self._update_instruments:
                    ...
                    futures[
                        executor.submit(self._dump_bin, _df, self._update_calendars)
                    ] = _code
                else:
                    # new stock
