...
db = SessionLocal()
# Check experiment 11 which just completed
experiment = db.query(Experiment).filter(Experiment.id == 11).first()
if experiment:
    ...
    if experiment.performance:
        ...
        if "cumulative_returns" in experiment.performance:
            ...
            if len(cum_returns) > 0:
                ...
                # Check if keys are proper dates
                first_key = list(cum_returns.keys())[0]
                print(f'First key type: {type(first_key)}')
                print(f'First key format: {first_key}')
